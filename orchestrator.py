...
def _fallback_plan(query: str) -> Dict:
    ...
    if any(k in q for k in ["news", "rumor", "notiz"]):
        plan.append({"agent": "analyst_news", "brief": f"Raccogli 3-5 notizie e sentiment su: '{query}' con fonti."})
    if any(k in q for k in ["tecnico", "rsi", "support", "resist", "grafico", "livelli"]):
        plan.append({"agent": "analyst_tech", "brief": f"Analizza '{query}' su 1D e 4H: RSI, MA50/200, supporti/resistenze, volumi."})
    if any(k in q for k in ["fondament", "utili", "eps", "p/e", "guidance", "multipli"]):
        plan.append({"agent": "analyst_fund", "brief": f"Riepiloga 4 trimestri e multipli per '{query}': EPS sorpresa, revenue YoY, P/E vs peer, rischi."})
    if any(k in q for k in ["portafoglio", "ribilanci", "rischio", "allocazione"]):
        plan.append({"agent": "portfolio_mgr", "brief": f"Valuta impatto su portafoglio e possibili ribilanci relativi a: '{query}'."})
    if not plan:
        plan = [
            {"agent": "analyst_tech", "brief": f"Analisi tecnica base per '{query}' su 1D/4H."},
            {"agent": "analyst_news", "brief": f"News recenti e sentiment per '{query}' con 2-3 fonti."},
        ]
